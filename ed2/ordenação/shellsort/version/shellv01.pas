...
procedure knuth(var valorGap:integer);
var
	h: integer;
begin
	h := 1; //começar com 1, diminui uma repetição
	while(h < MAX) and (MAX > (h*3+1)) do
	begin 
    h := 3*h+1;
  end;
  writeln(h);
...
begin
	...
	gap := (MAX div 2) +1; //gap de shell
	//knuth(gap);
	
	for h := gap downto 1 do
	begin
