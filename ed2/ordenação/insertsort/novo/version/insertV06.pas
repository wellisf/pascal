...
begin
	...
	{$i+}
		...
		if ioresult <> 0 then writeln('	Error 404. 		<<<  Not File >>>')else
		begin
			...
			begin
				...
				begin
								   ///// aux := lista;  ////
									/// achou := false; ////
					while lista^.proximo <> nil do// while (auxpercorrer^.proximo <> nil) and (not achou) do
					begin
											if valor < auxpercorrer^.dado then
											begin
													achou := true;
													// faz a inserção...
													///
											...
											begin
												 lista := lista^.proximo; // auxpercorrer := auxpercorrer^.proximo;
											end;
